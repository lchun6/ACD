...
class Net(nn.Module):
    ...
    def forward(self, stu_id, input_exercise, input_knowledge_point):
        ...
        # prednet
        input_x = e_difficulty * \
            (stat_emb - k_difficulty) * input_knowledge_point
...
class NCDM(CDM):
    ...
    def train(self, train_data, test_data=None, epoch=10, device="cpu", lr=0.02, silence=False):
        ...
        for epoch_i in range(epoch):
            ...
            for batch_data in tqdm(train_data, "Epoch %s" % epoch_i):
                ...
                pred: torch.Tensor = self.ncdm_net(
            ...
            if test_data is not None:
                obj_acc, obj_auc, obj_rmse, obj_mae = self.eval(test_data, device=device)
                print("[Epoch %d] obj_acc: %.6f,obj_auc: %.6f,obj_rmse: %.6f, obj_mae: %.6f" %
                      (epoch_i, obj_acc, obj_auc, obj_rmse, obj_mae))
