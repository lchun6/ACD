...
save_list = ('frcsub/','a0910/', 'a2017/', 'junyi/', 'math_ec/')
epochs_list = (2, 3, 1, 1,1)
dataSet_idx = 4
test_ratio = 0.2
batch_size = 32
...
train_set, test_set = [
    ...
    for data in [train_data.reset_index(), test_valid.reset_index()]
]

logging.getLogger().setLevel(logging.INFO)
cdm = NCDM(knowledge_n, item_n, user_n)
cdm.train(train_data=train_set, test_data=test_set,epoch=epochs, device=device)
...
np.savetxt('./output/' + save_list[dataSet_idx] + "test_record.csv", test_record.reset_index(), fmt='%.6f',delimiter=',')